/* ============================================================
   ...
   - Base de datos en localStorage (seed = data.js)
...
function loadDB() {
  ...
  if (raw) {
    try { return JSON.parse(raw); } catch (e) { /* sigue a seed */ }
  }
  const seed = {
    season: SEASON_LABEL,
    teams: structuredClone(DEFAULT_TEAMS),
    drivers: structuredClone(DEFAULT_DRIVERS).map(d => ({ ...d, recentPositions: [] })),
